...
public class TotalReportBO {
	...
	public TotalReportBO() {
	 ...
	 public int getP90_PJ_STAT_ANALYSIS(Connection conn,String statisticdate)throws Exception{
	     ...
	     public int insert_t07_inrep_by_acct_mid(Connection conn,String statisticdate,String case_type,String granularity)throws Exception{
	   	 ...
	   	 if(case_type.equals("1")){
	   	  ...
	   	  sql="insert into t07_inrep_by_acct_mid(organkey,report_dt,currency_cd,cast_type,total_num," +
				...
				"COUNT(DISTINCT(CASE WHEN T3.PARTY_CLASS_CD = 'C' THEN T2.REPORTKEY ELSE NULL END)) AS TOTAL_NUM, " + 	// 对公账户大额或可疑交易报告份数
				"SUM(CASE WHEN T3.PARTY_CLASS_CD = 'C' THEN T3.CRAT ELSE 0 END) AS TOTAL_VAL, " + 	// 涉及总金额
				"COUNT(CASE WHEN T3.PARTY_CLASS_CD = 'C' THEN T3.TICD ELSE NULL END) AS ALERT_NUM, " + 	// 涉及交易笔数
				"COUNT(DISTINCT(CASE WHEN T3.PARTY_CLASS_CD = 'I' THEN T2.REPORTKEY ELSE NULL END)) AS SUCCESS_NUM, " + 	// 个人账户大额或可疑交易报告份数
				"SUM(CASE WHEN T3.PARTY_CLASS_CD = 'I' THEN T3.CRAT ELSE 0 END) AS ALERTNVL, " + 	// 涉及总金额
				"COUNT(CASE WHEN T3.PARTY_CLASS_CD = 'I' THEN T3.TICD ELSE NULL END) AS HANDOUT_NUM " + 	// 涉及交易笔数
		...
		" SELECT T2.CREATE_ORG,"+func.vch2dt(statisticdate, "yyyy-mm-dd")+",T3.CURR_CD,'1',"+
		"COUNT(DISTINCT(CASE WHEN T3.PARTY_CLASS_CD = 'C' THEN T2.REPORTKEY ELSE NULL END)) AS TOTAL_NUM, " + 	// 对公账户大额或可疑交易报告份数
		"SUM(CASE WHEN T3.PARTY_CLASS_CD = 'C' THEN ((T4.USD_FX_RATE*T3.CRAT)/T4.CURRENCY_UNIT) ELSE 0 END) AS TOTAL_VAL, " + 	// 涉及总金额
		"COUNT(CASE WHEN T3.PARTY_CLASS_CD = 'C' THEN T3.TICD ELSE NULL END) AS ALERT_NUM, " + 	// 涉及交易笔数
		"COUNT(DISTINCT(CASE WHEN T3.PARTY_CLASS_CD = 'I' THEN T2.REPORTKEY ELSE NULL END)) AS SUCCESS_NUM, " + 	// 个人账户大额或可疑交易报告份数
		"SUM(CASE WHEN T3.PARTY_CLASS_CD = 'I' THEN ((T4.USD_FX_RATE*T3.CRAT)/T4.CURRENCY_UNIT) ELSE 0 END) AS ALERTNVL, " + 	// 涉及总金额
		"COUNT(CASE WHEN T3.PARTY_CLASS_CD = 'I' THEN T3.TICD ELSE NULL END) AS HANDOUT_NUM " + 	// 涉及交易笔数
...
//			"COUNT(DISTINCT(CASE WHEN T2.PARTY_CLASS_CD = 'C' THEN T2.REPORTKEY ELSE NULL END)) AS TOTAL_NUM, " + 	// 对公账户大额或可疑交易报告份数
//			"0 AS TOTAL_VAL, " + 	// 涉及总金额
//			"0 AS ALERT_NUM, " + 	// 涉及交易笔数
//			"COUNT(DISTINCT(CASE WHEN T2.PARTY_CLASS_CD = 'I' THEN T2.REPORTKEY ELSE NULL END)) AS SUCCESS_NUM, " + 	// 个人账户大额或可疑交易报告份数
//			"0 AS ALERTNVL, " + 	// 涉及总金额
//			"0 AS HANDOUT_NUM " + 	// 涉及交易笔数
