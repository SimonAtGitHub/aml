...
public class CaseBO
{
  ...
  public void UpdateAlertLevel(Connection conn, String statisdate, String flag) throws Exception
  {
    ...
    if ((flag != null) && (flag.equals("1"))) {
      sql = "UPDATE T07_CASE_APPLICATION A SET A.ALERT_LEVEL='02' WHERE    A.CASE_DATE=" + 
        this.func.vch2dt(statisdate, "yyyy-mm-dd");
      SQLExecute.exeSql(conn, sql);
    }

     sql = "UPDATE T07_CASE_APPLICATION A SET A.ALERT_LEVEL='03' WHERE EXISTS( SELECT 'X' FROM V07_BLACKLIST B WHERE B.PARTY_ID=A.PARTY_ID AND B.ISUSE='0' AND B.ISCHECK='1'  ) AND  A.CASE_DATE=" + 
      this.func.vch2dt(statisdate, "yyyy-mm-dd");
    SQLExecute.exeSql(conn, sql);
  }
  ...
  public int updateT07_IsWrite(Connection conn, String flag, String statisdate) throws Exception {
    ...
    if (flag.equals("1"))
    {
      ...
      sql = "UPDATE T07_CASE_APPLICATION A SET A.APP_STATE_CD='4',A.IS_DELFLAG='1'  WHERE A.IS_MENU='1' AND A.CAST_TYPE='1' AND DATE_STATUS_CD='2'  AND EXISTS (SELECT 'X' FROM t07_whitelist_info B WHERE B.PARTY_ID=A.PARTY_ID AND B.ISCHECK='1' AND B.ISUSE='0' and b.RULE_TYPE in('1','3') )   AND   A.CASE_DATE=" + 
        this.func.vch2dt(statisdate, "yyyy-mm-dd");
      count = SQLExecute.exeSql(conn, sql);
      //zyd 2018-08-27  苏宁白名单排除可疑
      sql = "UPDATE T07_CASE_APPLICATION A SET A.APP_STATE_CD='4',A.IS_DELFLAG='1'  WHERE A.IS_MENU='1' AND A.CAST_TYPE='2' AND DATE_STATUS_CD='2'  AND EXISTS (SELECT 'X' FROM t07_whitelist_info B WHERE B.PARTY_ID=A.PARTY_ID AND B.ISCHECK='1' AND B.ISUSE='0' and b.RULE_TYPE in('2','3') )   AND   A.CASE_DATE=" + 
      this.func.vch2dt(statisdate, "yyyy-mm-dd");
      count = SQLExecute.exeSql(conn, sql); 
     // System.out.println("苏宁白名单排除  更改t07_case_applicatin 状态"+count+"行");
      logger.debug("苏宁白名单排除  更改t07_case_applicatin 状态"+count+"行");
	  ...
	  +"   AND  A.STATUS='1' AND  A.CASE_DATE="+func.vch2dt(statisdate, "yyyy-mm-dd");
       count=SQLExecute.exeSql(conn, sql);
      // System.out.println("苏宁白名单排除  更改T07_CASE_STCR_KY 状态"+count+"行");
       logger.debug("苏宁白名单排除  更改T07_CASE_STCR_KY 状态"+count+"行");
     sql="UPDATE T07_CASE_TRANSACTION_KY A SET A.STATUS_CD='4'"
      ...
      +" where  B.PARTY_ID=A.PARTY_ID AND B.ISCHECK='1' AND B.ISUSE='0'  and b.RULE_TYPE in('2','3'))"
       ...
       count=SQLExecute.exeSql(conn, sql);	
     //  System.out.println("苏宁白名单排除  更改T07_CASE_TRANSACTION_KY 状态"+count+"行");
       logger.debug("苏宁白名单排除  更改T07_CASE_TRANSACTION_KY 状态"+count+"行");
   //end
    }
