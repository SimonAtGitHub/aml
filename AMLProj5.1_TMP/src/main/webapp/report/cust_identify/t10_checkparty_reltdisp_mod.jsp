	...
	var today=d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate();
   	if(getLength(document.forms[0].valid_dt_disp.value)==0){
        ...
        document.forms[0].valid_dt_disp.focus();
  		...
  		document.forms[0].valid_dt_disp.focus();
  	}else if(getLength(document.forms[0].check_result.value)==0){
		errMsg+="调查结论不能为空！";
        document.forms[0].check_result.focus();
	}
	//else if(getLength(document.forms[0].auditlevel_flag.value)==0){
		//errMsg+="请选择是否调整风险等级！";
       // document.forms[0].auditlevel_flag.focus();
	//}
...
function check_listtype(){
	var result_val=document.forms[0].check_result.value;
	var list_type_val=document.forms[0].list_type.value;
	//alert(result_val);
	//alert(list_type_val);
	
	//result_val:0,无需 1一般,2重点
	//list_type_val:1已在白名单 ,2已在黑明白,3不在任何名单
	if(result_val == 0)
	{
		if(list_type_val == 1)
		{
			document.getElementById("w_flag1").style.display='none';
			document.getElementById("w_flag2").style.display='none';
			document.getElementById("b_flag1").style.display='none';
			document.getElementById("b_flag2").style.display='none';
		}
		else if(list_type_val == 2 || list_type_val == 3)
		{
			document.getElementById("w_flag1").style.display='';
			document.getElementById("b_flag1").style.display='none';
			document.getElementById("b_flag2").style.display='none';
		}
	...
	else if(result_val == 2)
	{
		...
		else if(list_type_val == 1 || list_type_val == 3)
		{
			document.getElementById("w_flag1").style.display='none';
			document.getElementById("w_flag2").style.display='none';
			document.getElementById("b_flag1").style.display='';
		}
...
<html:form action="/cust_identify/t10_checkparty_reltdisp_mod.do" method="post">
<div class="conditions">  
    ...
    <table border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
		...
		<logic:equal name="t10_checkparty_funActionForm" property="auditlevel_flag" value="0">
		   <html:hidden property="auditlevel_flag"/>
		</logic:equal>
	    <logic:notEqual name="t10_checkparty_funActionForm" property="auditlevel_flag" value="0">
		<td><font color="red">*</font>是否调整风险等级：</td>
		<td>
			<html:select property="auditlevel_flag" style="width:300" onchange="check_level();">
				<html:options collection="auditlevel_flagMap" property="label" labelProperty="value" />
			</html:select>
		</td>
		</logic:notEqual>
	  </tr>
	  ...
	  <tr>
	  	<td><font color="red">*</font>调查结论：</td>
		<td>
			<html:select property="check_result"  style="width:300"  onchange="check_listtype();">
				<html:options collection="check_resultMap" property="label" labelProperty="value" />
			</html:select>
		</td>
		<td id="risklevel1" style="display:none">风险等级：</td>
		<td id="risklevel2" style="display:none">
			<html:select property="auditlevel" style="width:300">
				<html:options collection="auditlevel_Map" property="label" labelProperty="value" />
			</html:select>
		</td>
	  </tr>
	  -->
	  ...
	  <tr id="w_flag1" style="display:none">
		<td >是否白名单：</td>
		<td colspan="3">
	  ...
	  <tr id="w_flag2" style="display:none">
		...
		<td>
			<html:select property="w_rule_type" style="width:300">
		...
		<td bgcolor="F6F9FF" align="left">
				<html:textarea cols="29" rows="3" property="w_reason"></html:textarea>
				<br>
		</td>
	  </tr>
	  ...
	  <tr id="b_flag2" style="display:none">
		...
		<td>
			<html:select property="m_list_type" style="width:250">
				<html:options collection="m_list_typeMap" property="label" labelProperty="value" />
			</html:select>
		</td>
		<td>黑名单建立原因：</td>
		<td bgcolor="F6F9FF" colspan="3">
				<html:textarea cols="29" rows="3" property="b_reason"></html:textarea>
				<br>
		</td>
	  </tr>
