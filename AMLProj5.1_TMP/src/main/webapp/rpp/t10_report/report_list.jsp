		...
		<html:form method="post" action="/t10_report/t10_report_list.do">
			<div class='awp_title'>
				<table>
					<tr>
						<td width="23%">
							<span class="awp_title_td2">报表定义 - 列 表</span>
						</td>
						<td align="right">
							<input type="button" name="btsearch2" value="查 找"
								onclick="btnDisplayHidden(this, 'searchtable')" />
							<input type="button" name="add" value="新 建"
								onClick="dosubmit('t10_report_add.do','add')" class="input">
							<input type="button" name="modify" value="修 改"
								onClick="dosubmit('t10_report_modify.do','modi')" class="input">
							<input type="button" name="delete" value="删 除"
								onclick="dosubmit('t10_report_delete_do.do','del')"
								class="input">
							<input type="button" name="delete" value="申请发布"
								onclick="dosubmit('t10_report_deploy.do?deployflag=2&type=apply','deploy')"
								class="input">
					...
					<tr>
						...
						<td>
							<input type="button" name="button" value="查 询"
								onClick="dosubmit('t10_report_list.do?newsearchflag=1','search')"
								class="input">
						</td>
					...
					<logic:iterate id="report" name="t10_reportList"
						...
						<tr>
							...
							<logic:notEqual value="1" name="report" property="ishavepara">
								<a	href="javascript:openWin('<%=request.getContextPath()%>/rpp/showReport.do?new_qry_flag=1&jump_rslt_flag=1&fast_qry=1&reportkey=<%=report.getReportkey()%>&p_reportkey=<%=report.getReportkey()%>&isout=1','','600','800')">
									<bean:write name="report" property="reportkey" scope="page" />
								</a>
							</logic:notEqual>
							<logic:equal value="1" name="report" property="ishavepara">
								<a	href="javascript:openWin('t10_report_test.do?reportkey=<%=report.getReportkey()%>&p_reportkey=<%=report.getReportkey()%>&isout=1','','600','800')">
									<bean:write name="report" property="reportkey" scope="page" />
								</a>
							</logic:equal>
							</td>
							<td align="center">
								<a
									href="javascript:dosubmit('<%=request.getContextPath()%>/rpp/t10_report/t10_report_modify.do?reportkey=<%=report.getReportkey()%>','add')">
