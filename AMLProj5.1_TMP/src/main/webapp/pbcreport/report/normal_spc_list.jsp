...
function pbcTypechange(){
    ...
    var url="";
    if(index1=="R"){
        url='<%=request.getContextPath()%>'+"/pbcreport08/report/resend_spc_list.do?pbc_msg_type_cd=R&newsearchflag=2";
    }
    if(index1=="I"){
        url='<%=request.getContextPath()%>'+"/pbcreport08/report/revise_spc_list.do?pbc_msg_type_cd=I&newsearchflag=1";
    }
    if(index1=="C"){
        url='<%=request.getContextPath()%>'+"/pbcreport/report/normal_spc_list.do?pbc_msg_type_cd=C&newsearchflag=1&flag=1";
    }
...
可疑正常报告
	 ...
	 <span class="buttons">
						  ...
						  <a href="#" id="checkall" ><img src="../../images/<%=session.getAttribute("style") %>/b_checkall.png" />全选</a>  
						    <a href="#" onclick="dosubmit('<%=request.getContextPath()%>/pbcreport/report/createPacket.do?pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />&packet_type_cd=N&create_type=0&interfacekey=BS&url=<%=request.getContextPath()%>/pbcreport/report/normal_spc_list.do?pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />','create')" >
						 <img src="../../images/<%=session.getAttribute("style") %>/b_generatec.png" />选择生成数据包</a>
						 <logic:equal value="1" name="is_allPacket">
						    <a href="#" onclick="dosubmit('<%=request.getContextPath()%>/pbcreport/report/make_normal_spcPacketDo.do','all')" >
						<img src="../../images/<%=session.getAttribute("style") %>/b_generate.png" />全部生成数据包</a>	
					     </logic:equal>
					     <logic:notEqual value="1" name="is_allPacket">
					       <a href="#"  onclick="dosubmit('<%=request.getContextPath()%>/pbcreport/report/createPacket4.do?pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />&packet_type_cd=N&create_type=1&interfacekey=BS&url=<%=request.getContextPath()%>/pbcreport/report/normal_spc_list.do?pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />','all')" >
						   <img src="../../images/<%=session.getAttribute("style") %>/b_generate.png" />全部生成数据包</a>
					     </logic:notEqual>
  					     <a href="#"  onclick="returnOpertor('<%=request.getContextPath()%>/pbcreport/report/return_normal_spc_t07_report.do','return')" >
	  	...
	  	<tr >
	   		<td width="10%" colspan="1" class="in_button" >
									...
									<input type="button" name="button1" value="大额" class="input"
											onClick="dosubmit('normal_huge_list.do?newsearchflag=2&pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />','change')"/>
								  ...
								  <a href="#" onclick="window.open('<%=request.getContextPath()%>/common/t00_organ_search.do?newsearchflag=1&input_name=create_org&input_name_disp=create_org_name','enter2','height=400, width=600,left=100,top=100,scrollbars=yes,resizable=yes');" ><span class="cond_buttons"><img src="../../images/<%=session.getAttribute("style") %>/b_location.png" />定位</span></a>
									...
									<td bgcolor="F6F9FF">
										...
										<input type="button" name="button3" value="查 询" class="in_button1"
											onclick="dosubmit('<%=request.getContextPath()%>/pbcreport/report/normal_spc_list.do?newsearchflag=1','search')" />
									</td>
								</tr>
   ...
   <table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
              <td height="8"><html:errors/><font color="#FF0000"><%=StringUtils.nullToStr((String)request.getAttribute("message"))%></font></td>
            </tr>
  </table>
