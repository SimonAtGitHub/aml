...
function pbcTypechange(){
    ...
    if(index1=="C"){
        url='<%=request.getContextPath()%>'+"/pbcreport/report/cbh_spc_list.do?pbc_msg_type_cd=C&newsearchflag=1";
    }
...
-->
	...
	<script type="text/javascript" src="../../js/calendar.js"></script>
		<html:form action="/report/cbh_spc_list.do" method="post">
						<div id="main">
<!-- conditions --> 
  <div class="conditions">
   <div class="cond_t"><span>可疑纠错<%=com.ist.util.Constans.REPORT_NAME %>
	</span>
	 <span class="buttons">
				 
					 <a href="#" id="checkall" ><img src="../../images/<%=session.getAttribute("style") %>/b_checkall.png" />全选</a>
					  <a href="#" onclick="dosubmit('<%=request.getContextPath()%>/pbcreport/report/createPacket.do?url=<%=request.getContextPath()%>/pbcreport/report/cbh_spc_list.do?pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />&packet_type_cd=S&create_type=0&interfacekey=BS','create')" >
						 <img src="../../images/<%=session.getAttribute("style") %>/b_generatec.png" />选择生成数据包</a>
					<a href="#" onclick="dosubmit('<%=request.getContextPath()%>/pbcreport/report/createPacket4.do?url=<%=request.getContextPath()%>/pbcreport08/report/cbh_spc_list.do?pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />&packet_type_cd=S&create_type=1&interfacekey=BS','all')" > 
	  	...
	  	<tr >
	   		<td width="10%" colspan="1" class="in_button" >
									<input type="button" name="button1" value="大额" class="input"
											onClick="dosubmit('correct_huge_list.do?newsearchflag=2&pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />','change')"/>
								...
								<a href="#" onclick="window.open('<%=request.getContextPath()%>/common/t00_organ_search.do?newsearchflag=1&input_name=create_org&input_name_disp=create_org_name','enter2','height=400, width=600,left=100,top=100,scrollbars=yes,resizable=yes');" ><span class="cond_buttons"><img src="../../images/<%=session.getAttribute("style") %>/b_location.png" />定位</span></a>
									...
									<td bgcolor="F6F9FF">
										...
										<input type="button" name="button3" value="查 询" class="in_button1"  
											onclick="dosubmit('<%=request.getContextPath()%>/pbcreport/report/cbh_spc_list.do?newsearchflag=1&pbc_msg_type_cd=<bean:write name="pbc_msg_type_cd" />','search')" />
									</td>
								</tr>
