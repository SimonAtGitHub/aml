...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
	  ...
	  function openMfa(net_id,type){
		
		var url='<%=request.getContextPath()%>/mfa/rel_issue/REL.html?';
		...
		{
		  var organ= '<%=((AuthBean) session.getAttribute("authBean")).getAuthToken().getT00_user().getOrgankey()%>';
		  url+='organ='+organ;
		}
	...
	<body class="mainbg">
		<html:form action="/relation/relation_net_list.do" method="post">
			<html:errors />
			<div id="main">
				<!-- conditions -->
				<div class="conditions">
					...
					<div class="cond_t">
						<span>关系网络处理</span>
						<span class="buttons"> 
							<a href="#" onclick="openMfa('','add')"><img
									src="../../images/<%=session.getAttribute("style")%>/b_add.png" />新建</a>
							<a href="#" onclick="dosubmit('<%=request.getContextPath() %>/mfa/relation/relation_delete.do','del')"><img
									src="../../images/<%=session.getAttribute("style")%>/b_delete.png" />删除</a> 
						</span>
					</div>
					<!-- table content -->
							...
							<tr>
								...
								<td>
									<input type="button" class="in_button1" name="search"
										value="查 询"
										onclick="dosubmit('relation_net_list.do?newsearchflag=1','search')" />
								</td>
							</tr>
						</table>
				...
				<div class="list">
					<table>
						...
						<logic:iterate id="t13_relation_net" name="t13_relation_netList"
							...
							<tr>
							    <td>
									<html:multibox property="selectedNetId">
										<bean:write name="t13_relation_net" property="net_id" />
									</html:multibox>
								</td>
