...
<c:if test="${showMode == 'edit'}">
    <p align="left">
        <input type=button value="添 加 附 件" name="add" class="in_button1"
            onClick="uploadfile_n('<%=request.getContextPath()%>/cmp/attachment/t01_attachment_add.do?attachtype=<c:out value="${attachtype}" />&systemkey=<c:out value="${systemkey}" />')">
    </p>
                ...
                <TR align="center">
                    ...
                    <TD>
                        <c:if test="${currentUser == attachList['creator']}">
                            <a href="javascript:void(0);"
                                onclick="editAttach('<%=request.getContextPath()%>/cmp/attachment/t01_attachment_control.do?attachkey=<c:out value="${attachList['attachkey']}" />&isMark=false&isEmbedded=false')">编辑
                            </a>
					          &nbsp;
