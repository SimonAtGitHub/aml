...
function MzTreeView(Tname)
{
  ...
  this.toString = function()
  {
    ...
    var str = "<A id='"+ this.name +"_RootLink' href='#'></A>";
...
MzTreeView.prototype.setStyle = function()
{
  ...
  */
  var style = "<style>"+
  //"DIV.MzTreeView DIV{margin-top:10px;} "+
  "DIV.MzTreeView DIV IMG{border: 0px solid #FFFFFF;}"+
  "DIV.MzTreeView DIV SPAN IMG{border: 0px solid #FFFFFF;}"+
  "DIV.MzTreeView A:link {text-decoration:none; color:#036A79}" +
  "DIV.MzTreeView A:visited{text-decoration: none; color: #036A79}" ;
  if(this.wordLine)
  {
    style +="\
    DIV.MzTreeView DIV\
    {\
      ...
      (this.navigator=="netscape" ? "" : "overflow: ;" ) +"\
    }\
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var input = this.getAttribute(source, "input");
  var checked = this.getAttribute(source, "checked");
  var path = this.node[parentId].path + d + this.index
  ...
  var chks = path.split(d);
  
  for(var i=0; i<chks.length; i++){
	  var tmpid = chks[i] < 0 ? 0 :chks[i];
	  this.node[tmpid].cid +=  (d + this.index);
  } 
...
HTML += "<IMG "+
  ...
  if(node.input == 'checkbox') {
	  HTML += "<input type='checkbox' name='awp_tree' value='"+node.sid+"' id='input_"+node.id+"' "+node.checked+" disp_val ='"+node.text+"' >";
  }
  
  if(node.input == 'radio'){
	  HTML += "<input type='radio' name='awp_tree' value='"+node.sid+"' "+node.checked+" disp_val ='"+node.text+"'>";
  }
...
/***/
MzTreeView.prototype.selected = function(val){
	var sourceval = val.split(',');
	for(var i=0; i<sourceval.length; i++){
		this.checked(sourceval[i]); 
	}
}

MzTreeView.prototype.checked = function(sourceId){
	var path = this.getPath(sourceId);
	var root = this.node["0"], len = path.length;
	var currSourceId = path[len-2] + this.divider + path[len-1];
	this.nodes[currSourceId]+= ";checked:checked";
	for(var i=1; i<len; i++){
		if(root.hasChild){
...
//点击展开树节点的对应方法
MzTreeView.prototype.expand   = function(id, sureExpand,iconOpen)
{
  ...
  if (area)
  {
    ...
    if (expand)
    {
	  ...
	  if(iconOpen){ var srcnew=this.iconsExpand[iconOpen].src;setTimeout(function(){ expand.src=srcnew;  },20);}//pengpeng 2014/6/13 expandAll
    }
...
MzTreeView.prototype.nodeClick = function(id)
{
  ...
  var nodeUrl = this.getAttribute(source, "url");
   if(nodeUrl == "#"){
	  return false;
  }else{
	  return !(!nodeUrl && this.url=="#");
  }
};
...
MzTreeView.prototype.clickHandle = function(e)
{
  ...
  switch(e.tagName)
  {
    ...
    case "A" :
      ...
      break;
    case "INPUT" :
		//this.expandAllSubNode(e.id.substr(e.id.lastIndexOf("_") + 1));
		//this.selectedDownChk(e.id.substr(e.id.lastIndexOf("_") + 1));
		//this.selectedUpChk(e.id.substr(e.id.lastIndexOf("_") + 1));
		//this.selectedDownChkOneLev(e.id.substr(e.id.lastIndexOf("_") + 1));
      break;
    case "SPAN" :
      if(e.className=="pm")
        this.expand(e.id.substr(e.id.lastIndexOf("_") + 1));
...
MzTreeView.prototype.expandAll = function()
{
  ...
  while(e.id != "0")
  {
    ...
    else
    {
      if(e.hasChild && !isdo)
      {
        this.expand(e.id, true,'PM3'); t = false;
...
/**选中上级*/
MzTreeView.prototype.selectedUpChk= function(id){
	var c_chk = this.getElementById('input_'+id);
	if(!c_chk.checked){
		return ;
	}
	var chks =this.node[id].path.split(this.divider);
    var e_chk = "";
	for(var i=0; i<chks.length; i++)
    {
		e_chk = this.getElementById('input_'+chks[i]);
		if(e_chk){
			e_chk.checked = true;
		}
    } 
...
MzTreeView.prototype.selectedDownChk= function(id){
	var c_chk = this.getElementById('input_'+id);
	
    
	var chks =this.node[id].cid.split(this.divider);
    var e_chk = "";
	for(var i=0; i<chks.length; i++)
    {
		e_chk = this.getElementById('input_'+chks[i]);
		if(e_chk){
			e_chk.checked = c_chk.checked;
		}
    } 
};
//只选中下一级
MzTreeView.prototype.selectedDownChkOneLev= function(id){
	var c_chk = this.getElementById('input_'+id);
	var tmp_e = this.node[id];
	
	if(c_chk){
		if(tmp_e.hasChild ) {
			if (!tmp_e.isExpand){
				this.expand(tmp_e.id, true);
			}
		}else{
			return ;
		}
	
		for(var i=0; i<tmp_e.childNodes.length; i++){
			var e_chk = this.getElementById('input_'+tmp_e.childNodes[i].id);
			if(e_chk){
				e_chk.checked = c_chk.checked;
			}
		}
...
//展开某节点的所有子节点
MzTreeView.prototype.expandAllSubNode = function(id)
{
	var c_chk = this.getElementById('input_'+id);
	if(!c_chk.checked){
		return ;
...
MzTreeView.prototype.setCheckedVal     = function(chkVal){
	var chks = chkVal.split(",");
};
...
function _resetAll() {
	var text = "problemsText";
	var code = "problemsCode";
	var checkbox = document.getElementsByTagName("input");
	for (i = 0; i < checkbox.length; i++) {
		checkbox[i].checked = false;
	}
	var ctext = document.getElementById(text);	//存储当前选中的父节点内容	
	var cCode = document.getElementById(code);	//存储当前选中的父节点所有内容的key值	
	ctext.value = "";
	cCode.value = "";
}

MzTreeView.prototype.expandNode = function(id){
    try{
	   this.expand(id, true);
	}catch(e){
		return;
