function TreeView(obj,target){
	...
	this.html=""
	this.config={
		root		:'../../images/img/web.gif',
		open		:'../../images/img/open.gif',
		close		:'../../images/img/close.gif',
		file		:'../../images/img/file2.gif',
		join		:'../../images/img/T1.gif',
		joinbottom	:'../../images/img/L1.gif',
		plus		:'../../images/img/Tplus.gif',
		plusbottom	:'../../images/img/plus_end.gif',
		minus		:'../../images/img/Tminus.gif',
		minusbottom	:'../../images/img/minus_end.gif',
		blank		:'../../images/img/empty.gif',
		line		:'../../images/img/I1.gif'
	}
...
TreeView.prototype.DrawNode=function(nid){
    if(nid==0){//用于隐藏第一个“-1”的显示
        var str=""
        var nNode=this.nodes[nid]
        this.DrawLine(nNode,nNode)
        if(nNode.hasNode)
        nNode.indent+="<a  href='' onclick='"+this.obj+".Click("+nid+",this);return(false)'>"
        nNode.indent+=this.root.id!=nNode.pid?("<img border=0 align='absmiddle' src='"+(nNode.lastNode?(nNode.hasNode?this.config.plusbottom.src:this.config.joinbottom.src):(nNode.hasNode?this.config.plus.src:this.config.join.src))+"' id='icon"+this.obj+nid+"'>"):""
        nNode.indent+="<img border=0 id='folder"+this.obj+nid+"' align='absmiddle' src='"+(nNode.pid==this.root.id?this.config.root.src:(nNode.hasNode?(nNode.open?this.config.open.src:this.config.close.src):this.config.file.src))+"'>"+(nNode.hasNode?"</a>":"")
        str+="<div class='node' style='display:none'><nobr>"+nNode.indent+this.DrawLink(nid)+"</nobr></div>\n"
        if(nNode.hasNode){
...
TreeView.prototype.DrawLine=function(nNode,tem){
	for(var i=1;i<this.nodes.length;i++){
		...
		nNode.indent="<img border=0 align='absmiddle' src='"+(this.nodes[i].lastNode?this.config.blank.src:this.config.line.src)+"'>"+nNode.indent
...
TreeView.prototype.OpenWin=function(nid){
	...
	if(nNode.link.indexOf("javascript:")==0){
	     eval(nNode.link);
	}else{
...
TreeView.prototype.DrawLink=function(nid){
	...
	//return "<span id='node"+this.obj+nid+"' class='normal' onmouseover='"+this.obj+".MouseOver(this)' onmouseout='"+this.obj+".MouseOut(this)' ondblclick='"+this.obj+".Click("+nid+")' onclick='"+this.obj+".selected("+nid+")'  title='"+nNode.title+"'>"+nNode.txt+"</span>"
        return "<span id='node"+this.obj+nid+"' class='normal' onmouseover='"+this.obj+".MouseOver(this)' onmouseout='"+this.obj+".MouseOut(this)' ondblclick='"+this.obj+".Click("+nid+")' onclick='"+this.obj+".selected("+nid+");"+this.obj+".Click("+nid+")'  title='"+nNode.title+"'>"+nNode.txt+"</span>"
}
