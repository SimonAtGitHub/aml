...
<link  href="<%=request.getContextPath()%>/skin/<%=style %>/css/awp_base.css" rel="stylesheet" type="text/css">
	...
	function updTimu(subjectid,typeId){	
	    sForm = document.frm   
	    sForm.subjectid.value = subjectid;
	    sForm.typeId.value = typeId;
...
<body>
	<div id='content'>
  	<html:form action="/t12_test_question_add_do.do" method="post">
  		...
  		<input type="hidden" name="delChoiceKey" value="" />
  		<!-- 操作来源 E-代表来源是修改页面 N代表来源是添加页面-->
  		<input type="hidden" name="fromOpt" value="<c:out value="${param.fromOpt}" />"/>
  		
  		<input type="hidden" name="subjectkey" value="<c:out value="${subject.subjectkey}" />"/>
		...
		<div class='awp_detail'>
  			<table id="subjectTable">
  				<tr>
  					...
  					<td noWrap><html:textarea property="content" cols="40" rows="5"></html:textarea></td>
  				...
  				<c:choose>
  					...
  					<tr>
	  					...
	  					<td noWrap><html:text property="score" size="5" onkeyup="this.value=this.value.replace(/\D/g,'')" maxlength="3"/></td> 
  					</tr>
  					</c:when>
  					<c:otherwise>
  				...
  				<c:set value="${subject.solution}" var="var_solution" scope="page"></c:set>
	  							<%
	  							String var_solution = (String)pageContext.getAttribute("var_solution");
	  							pageContext.setAttribute("solutions",var_solution.replace("|",","));
	  							 %>
  				<c:forEach items="${subject.choiceList}" var="choice" varStatus="status">
  					<tr>
  						<td>
  							<input type="checkbox" name="add_choice_id" value="<c:out value="${choice.choicekey}" />" 
  						...
  						<td>
  							<input type="text" name="choice_content" value="<c:out value="${choice.content}" />" />
  							<input type="hidden" name="add_choice_id_hidden" value="<c:out value="${choice.choicekey}" />" />
  							&nbsp;<a href='javascript:void(0)' onclick="delRow(event)"'>删除题干</a>
  						</td>
  					</tr>
  				</c:forEach>
  				<tr>
  					<td noWrap width="50%"> </td>
  					<td noWrap width="50%"><html:button property="Button52" value="添加选项" onclick="addrow()"/>&nbsp;
  					<html:button property="Button52" value="保 存" onclick="dosubmit('cms/t12_test_question_modify_do.do','modify')"/></td> 
  				</tr>
  			</table>
  		</div> 
  	</html:form></div>
  ...
  function addrow(){
		...
		var cell1 = newRow.insertCell();
			cell1.innerHTML="<input type=\"checkbox\" name=\"add_choice_id\" value=\""+add_value+"\">";
	...
	function delRow(event){
		if (confirm("确认删除")) {
		   ...
		   var eles = element.parentNode.childNodes;
		   for(var i=0;i<eles.length;i++){
			   if(eles[i].nodeType==1 && eles[i].name==='add_choice_id_hidden'){
			   	 	if(eles[i].value.indexOf('_')!=0){
			   	 		document.getElementsByName('delChoiceKey')[0].value+=eles[i].value+",";
			   	 	}
			   }
		   }
	       tab.deleteRow(rowNo);
