...
function dosubmit(theUrl,type){
   ...
   else if(type=='all')
   {
    ...
    <bean:size id="len" name="t37_party_resultList" />
   ...
   if(type=='search'){
   		if(!_compareTwoDateForString(document.forms[0].rcheck_dt_disp.value, document.forms[0].rcheck_dt_disp_end.value))
   		{
    		errMsg = "开始时间不能大于结束时间";	
    	}
    	else if (!_compareTwoDateForString(document.forms[0].statistic_dt_disp.value, document.forms[0].statistic_dt_disp_end.value))
    	{
    		errMsg = "开始时间不能大于结束时间";	
    	}
...
<BODY leftmargin="0" topmargin="0">
	<html:form action="/check/t37_curr_rate_result_list.do" method="post">
	<input type="hidden" name="backurl" value="../check/t37_curr_rate_result_list.do"/>
		...
		<div id="main">
			...
			<div class="conditions">
					<!-- title -->
				<div class="cond_t">
					<span>结果查询 - 当前评级结果</span>
					<span class="buttons">
						<a href="#" id="hdd_display" class="bt_hidden"></a>
						
						<a href="#" onclick="dosubmit('t37_curr_rate_result_export.do?&exportType=select','export')" />
						<img src="../../images/<%=session.getAttribute("style") %>/b_export.png" />导出客户风险信息</a>
						<a href="#" onclick="dosubmit('t37_curr_rate_result_exportall.do?&exportType=all','all')" />
				...
				<div class="cond_c" id="searchtable">
					<table width="100%" border="0" cellpadding="1" cellspacing="1">
							...
							<tr>
								...
								<td width="36%" >
								<html:text property="rcheck_dt_disp" onclick="calendar.show(this);" readonly="true" styleClass="calimg" size="12" />
                                 --
                                 <html:text property="rcheck_dt_disp_end" onclick="calendar.show(this);" readonly="true" styleClass="calimg" size="12" />
									</td>
							</tr>
							<tr>
                            ...
                            <tr >
								...
								<td width="37%" >
									...
									<input type="button" value="查  询" name=Button522
										onClick="dosubmit('t37_curr_rate_result_list.do?newsearchflag=1','search')"
										class="in_button1">
								</td>
							</tr>
						</table>
							...
							<logic:iterate id="result" name="t37_party_resultList"
								...
								<tr align="center" bgcolor="#FFFFFF">
								    ...
								    <%
											    ...
											    String resulekeys=resulekey+","+organkey;
										%>
										<input type="checkbox" name="selectedResulekey" value=<%=resulekeys %> />
									</td>
									<td nowrap>
									...
									<td>
										<bean:write name="result" property="nextstatistic_dt_disp" />
									</td>
									<td>
										<bean:write name="result" property="create_dt_disp" />
									</td>
									<td>
										<a href="#" onclick="dosubmit('<%=request.getContextPath()%>/risk_rate/result/t37_party_view_disp.do?resulekey=<%=resulekey %>&party_id=<%=party_id %>&party_class_cd=<bean:write name="result" property="party_class_cd" />&organkey=<bean:write name="result" property="organkey" />&temptype=<bean:write name="result" property="temptype" />&tempcategory=2','search')"><img src="../../images/<%=session.getAttribute("style")%>/b_caseinfo.png" title="详细信息" /></a> 
									</td>
