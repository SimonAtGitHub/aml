...
<html>
	<head>
		...
		function checkForms() {
			...
			var parmcate = document.forms[0].parmcate.value; //系统输入类型 当==2 或 3 时 tagetcod不能为空
				if ( parmcate == 2 ) {
					   var sysinparmkey_user_temp = document.getElementsByName("sysinparmkey_user");
			            ...
			            if(sysinparmkey_user_temp_value == '230' || sysinparmkey_user_temp_value == '240'|| sysinparmkey_user_temp_value == '250'){
				            ...
				            //列表定位  
							 if(getLength(document.forms[0].tagetcod.value)==0){
					             document.forms[0].tagetcod.focus(); 
					              alert("对应定位不能为空！");
					            return false;   
					          }
						}
	...
	<body>
		<div id="content">
			<html:form method="post" action="/search/t02_srch_eng_parm_modify_do.do">
				...
				<div class='awp_title'>
					<table>
						<tr>
							...
							<td align="right">
								<input type=button value="保 存" name=Button52 class="input" onClick="add('t02_srch_eng_parm_modify_do.do')">
								 <input type="button" value="返 回" onclick="awpDoSubmit('t02_srch_eng_parm_list.do');">   
							</td>
						</tr>
					</table>
				</div>
