...
<link  href="<%=request.getContextPath()%>/skin/<%=style %>/css/awp_base.css" rel="stylesheet" type="text/css">
		...
		function doSubmit(theUrl){
			...
			var begin_no = document.getElementById('begin_no');
			var end_no = document.getElementById('end_no');
			...
			if(hid_save == '2'){	
			 	if(getLength(document.forms[0].targetkey.value)==0){
			        alert("审计目标不能为空！");
			        document.forms[0].targetname.focus();
					return false;			        
			    }  
			    theredirtUrl =theredirtUrl+"?targetkey="+document.forms[0].targetkey.value;
			}
			dispLoadInfo1();
		...
		function chsaveMode(oElement){
			var tr_project = document.getElementById('tr_project');
			var tr_expbtn = document.getElementById('tr_expbtn');
			...
			if(hid_save == '1'){  //自选数据导出
				tr_project.style.display='none';
				tr_begin_no.style.display='block';
				tr_end_no.style.display='block';
				tr_expbtn.style.display='none';				
				tr_subbtn.style.display='block';
				tr_curr_subbtn.style.display='none';
			}else if ( hid_save == '2'){ //导入到清单
				tr_project.style.display='block';
				tr_begin_no.style.display='block';
				tr_end_no.style.display='block';
				tr_expbtn.style.display ='block';				
	...
	<BODY>
		<div id="content">
			<html:form method="post" action="/template/tpla_sort_cond.do">
				...
				<table>
					<tr>
						<td nowrap><span class="awp_title_td2"><c:out value="${labelStr}"></c:out>- 导出条件</span></td>
						<td align="right">
					...
					<table>
						 <tr>
				             ...
				             <td align="left">
								...
								<input type='radio' name="isexamsave" value="2" onclick="chsaveMode(this)"">导入到清单           
				             </td>
			            </tr> 	
						...
						<tr id="tr_project" style="display:none">
							<td width="20%"  height="22" align="right" nowrap="nowrap">
								<font color="#FF3300">*</font>选择导入审计目标：
							</td>
							<td align="left">                               
							    <input type="text" readOnly name="targetname" size="30" />	
							    <input type="hidden" name="targetkey" />					 
							    <input type="button" name="tool" onclick="locate_tree_window('<%=request.getContextPath() %>','project_checklist_tree','forms[0]','targetkey,targetname','lastradio','')" value="单 选" />
							</td>					
						</tr>					
						<tr id="tr_expbtn" style="display:none">
							<td width="20%" >
								&nbsp;
							</td>
							<td align="left">
								...
								<input type="button" value="导 入"
									onClick="doSubmit('importCheckListPreview.do')">
							</td>
						</tr>												
				...
				<input type="hidden" name="hid_save" />
...
<input   type=text   name=ha   style= "display:none ">  
	...
	if(document.all.ha.value   ==   1)   { 	
		var radios = document.forms[0].isexamsave;
