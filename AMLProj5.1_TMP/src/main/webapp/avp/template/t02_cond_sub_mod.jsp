...
<HTML>
	<HEAD>
		...
		{    //添加行
		    ...
		    tempNode.innerHTML = "<b><span id='a"+n+"' class='css012'>&nbsp;" +<%=request.getAttribute("pre_sb")%>+n+<%=request.getAttribute("sec_sb")%>+n+<%=request.getAttribute("third_sb")%>+n+<%=request.getAttribute("four_sb")%>+n+<%=request.getAttribute("fifth_sb")%>+n+<%=request.getAttribute("end_sb")%>+"&nbsp;<input type='button'  value='删除' onclick=delRow('a"+n+"')></span></b>";
		...
		{  
		   ...
		   <%ArrayList list=(ArrayList)request.getAttribute("condList");
		    ...
		    tempNode.style.marginTop="3 px";
		    tempNode.innerHTML = "<b><span id='a"+n+"' class='css012'>&nbsp;" +<%=request.getAttribute("pre_sb"+i)%>+n+<%=request.getAttribute("sec_sb"+i)%>+n+<%=request.getAttribute("third_sb"+i)%>+n+<%=request.getAttribute("four_sb"+i)%>+n+<%=request.getAttribute("fifth_sb"+i)%>+n+<%=request.getAttribute("end_sb"+i)%>+"&nbsp;<input type='button'  value='删除' onclick=delRow('a"+n+"')></span></b>";
		    divForm.appendChild(tempNode);//添加到divForm的子节点后面去
		    <%}%>
		}
		function delRow(id)
	...
	<input type="hidden" name="busstypecd" value="<%=request.getAttribute("busstypecd") %>"/>
	   <div class='awp_title'>
				<table>
					<tr>
						...
						<td width="23%" nowrap><span class="awp_title_td2">模型管理 - 定义跳转条件页面</span></td>
						<td align="right">
							<input class="input" type="button" onclick="coundByte();dosubmit('adT02_cond_subDo.do');" value="保 存">&nbsp;&nbsp;
	   ...
	   <div class='awp_dynamic_list'>
			<table>
				<tr>
		       <TD align="center" noWrap>定义跳转条件</TD>
		...
		<div id="divForm"  align="left" style='table-layer:fixed; overflow:auto'>
		</div>
