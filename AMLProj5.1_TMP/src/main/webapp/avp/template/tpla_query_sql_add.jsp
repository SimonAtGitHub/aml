...
<link  href="<%=request.getContextPath()%>/skin/<%=style %>/css/awp_base.css" rel="stylesheet" type="text/css">
	...
	function checkSql(sql){
	   var forbidword="insert,update,delete,drop,truncate,alert,create";
	   var sqlitArray = new Array(); 
       sqlitArray = forbidword.split(',');
       for(var i=0; i < sqlitArray.length; i++ )
       { 
               if(sql.toLowerCase().indexOf( sqlitArray[i] )>-1){
                    if(sql.toLowerCase().indexOf("_temporary_table")>-1){
                        return true;
                    }else{
                        return false;
                    }
...
function   savePos(oE)   
  ...
  {   
			...
			<div class='awp_title'>
				<table>
					<tr>
						...
						<td align="right">
							<input type=button value="保存查询SQL" name=Button223 class="input" onClick="save('t02_tp_exec_qry_add_do.do');">
				     		<!-- <input type=button value="预览查询结果" name=Button52 class="input" onClick="dosubmit('tpla_cal_col.htm')"> -->
	...
	<div class='awp_detail' >
		<table  >
			<tr >
				<td><font color="#FF0000">*</font>执行SQL：<br>
