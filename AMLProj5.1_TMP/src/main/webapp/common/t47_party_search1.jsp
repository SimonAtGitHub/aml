...
function _Click(party_id,party_chn_name,party_class_cd,organkey,card_type,card_no){
     ...
     document.forms[0].temp_party_class_cd.value=party_class_cd;
     document.forms[0].organkey.value=organkey;
     document.forms[0].card_type.value=card_type.substr(0,2);//修改证件号码只取前2位 modify by qiuyun 2014-02-13
     document.forms[0].temp_card_no.value=card_no;
...
function _Commit(){
    if(checkRadio(document.forms[0].party_id1)<0){
     ...
     opener.document.forms[0].<%=(String) request.getAttribute("type")%>.value=document.forms[0].temp_party_class_cd.value;
     var c_type=document.forms[0].temp_party_class_cd.value;
     var disp=document.forms[0].dispatcher.value;
     if(disp=="linkage"){
	     if(c_type=="C"){
	       opener.document.all.id_aml1.style.display='none' 
	     }else if(c_type=="I"){
	       opener.document.all.id_aml1.style.display=''
	     }
     }
     
     
     var f = opener.document.forms[0];
     if(typeof(f.organkey)!='undefined')
     	f.organkey.value=document.forms[0].organkey.value;
     if(typeof(f.card_type)!='undefined'){
     	f.card_type.value=document.forms[0].card_type.value;
     	}
     if(typeof(f.card_no)!='undefined')
     	f.card_no.value=document.forms[0].temp_card_no.value;
   window.close();
   }
}
</script>
	         ...
	         <html:text property="host_cust_id" styleClass="text_white"  size="12"/>
	          -->
									...
									<html:button property="button3" value="查 询"
										...
										onclick="dosubmit('t47_party_list1.do?newsearchflag=1&searchtype=search')" />
									<html:hidden property="party_chn_name" />
									<html:hidden property="party_id" />
									<input type="hidden" name="temp_party_class_cd"/>
									<html:hidden property="organkey" />
									<html:hidden property="card_type" />
									<input type="hidden" name="temp_card_no"/>
									<input type="hidden" name="dispatcher" value="<%=(String) request.getAttribute("dispatcher")%>"/>
								</td>
							</tr>
						</table>
					</div>
