...
<META content="MSHTML 6.00.2600.0" name=GENERATOR><title>企业查询</title>
...
function selectUsers(){
	...
	var obj=opener.document.forms(0).<bean:write name="input_name"/>;
        var usernameObj=document.forms(0).selectedUsername;

        if(typeof(usernameObj)=='object'){
        	if(usernameObj.length>1){
         	     for(var i=0;i<usernameObj.length;i++){
                          if(document.forms(0).selectedUsername[i].checked){
                              addOptionBy(obj,document.forms(0).selectedUsername[i].value,document.forms(0).realname[i].value);
                          }
                     }
      		  }
      		  else{
       	 	    if(usernameObj.checked){
                    	  addOptionBy(obj,document.forms(0).selectedUsername.value,document.forms(0).realname.value);
                    }
      		  }
        }
...
function addOptionBy(obj,value,text) {
    if (obj!=null) {
        var length=obj.length;
        if(length==null){
        	length=0;
        }
        //判断是否存在相同的VALUE,如果存在,则不进行添 加操作
        var flag=false;
...
<html:hidden property="input_name_disp"/>
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="0">
   ...
   <table width="98%" border="0" align="center" cellpadding="2" cellspacing="0">
	...
	<html:button property="s1" value="查  询" styleClass="text_white" onclick="dosubmit('t00_user_searchmulti.do?newsearchflag=1')"/>
...
<logic:iterate id="user" name="t00_userList" type="com.ist.platform.dto.T00_user">
    <TR align="center" class="interval" onMouseOver="this.className='interval2'" onMouseOut="this.className='interval'">
      <TD height=22>
          <html:hidden name="user" property="realname" />
