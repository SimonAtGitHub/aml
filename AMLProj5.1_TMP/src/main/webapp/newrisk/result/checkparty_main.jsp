...
<div id="bigTab">
	<dl class="tabm">
		<dd class="tabcur">
		    <span onclick="_chengeUrl('<%=request.getContextPath()%>/report/custcheck/t10_survey_cust_risk.do?newsearchflag=1&party_id=<%=party_id %>&party_class_cd=<bean:write name="t37_party_resultActionForm" property="party_class_cd" />')">客户信息</span>	                       
		</dd>
		<dd>
		    <span onclick="_chengeUrl('<%=request.getContextPath()%>/report/custcheck/modifyT10_colu.do?newsearchflag=1&party_id=<%=party_id %>&party_class_cd=<bean:write name="t37_party_resultActionForm" property="party_class_cd" />')">客户调查</span>	                       
		</dd>
		<dd>
		    <span onclick="_chengeUrl('<%=request.getContextPath()%>/report/cust_identify/t10_party_risklist_mod.do?newsearchflag=1&party_id=<%=party_id %>&party_class_cd=<bean:write name="t37_party_resultActionForm" property="party_class_cd" />')">风险事件</span>	                       
		</dd>
		<dd>
		    <span onclick="_chengeUrl('<%=request.getContextPath()%>/report/cust_identify/t10_checkparty_reltdisp_mod.do?newsearchflag=1&changeflag=<bean:write name="t37_party_resultActionForm" property="changeflag"/>&check_type=<bean:write name="t37_party_resultActionForm" property="check_type"/>&party_id=<%=party_id %>&party_class_cd=<bean:write name="t37_party_resultActionForm" property="party_class_cd" />&auditlevel_flag=0')">调查结论</span>	                       
		</dd>			
	</dl>
	<dl class="tabc">
	    <dd class="tabshow">
