				...
				<div class="conditions">
					<!-- title -->
						...
						<span><logic:equal value="P2001" name="t37_party_resultActionForm" property="post_id">
							      ...
							      <logic:equal value="2,3" name="t37_party_resultActionForm" property="tempcategory">
						                           定期客户审查
						          </logic:equal>
						...
						<span class="buttons">
						    <a href="#" id="hdd_display" class="bt_hidden"></a>&nbsp;
 		                    <logic:iterate id="operlist" name="workbench_operlist"
								type="com.ist.aml.newrisk.dto.T31_n_move">
								<a href="#"  name="<%=operlist.getOper_id()%>"
									onClick="opersubmit('t37_flow_workbench_oper.do?oper_id=<%=operlist.getOper_id()%>&node_id=<%=operlist.getNode_id()%>');">
				...
				<div class="list">
					<table border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<logic:notEqual value="1" name="t37_party_resultActionForm" property="tempcategory">
							</th>
								<th width="10%" nowrap>
								评级类型
							</th>
								...
								<tr align="center" bgcolor="#FFFFFF">
									<td align="center">
										...
										<%
											    ...
											    String templatekey=result.getTemplatekey();
											    String organkey=result.getOrgankey();
											  
											    String resulekeys=resulekey+","+higherlevel+","+templatekey+","+organkey;
										%>
									...
									<td>
										<bean:write name="result" property="dealmax_dt_disp" />
									</td>
									<td>
									...
									<td>
										   <a
											href="#" onclick="dosubmit('<%=request.getContextPath()%>/newrisk/result/t37_result_gs_modify.do?resulekey=<%=resulekey%>&templatekey=<bean:write name="result" property="templatekey" />&temptype=<bean:write name="result" property="temptype" />&party_id=<%=party_id %>&statistic_dt_disp=<bean:write name="result" property="statistic_dt_disp" />&organkey=<bean:write name="result" property="organkey"/>')"><img src="../../images/<%=session.getAttribute("style")%>/toolsicon02.gif" title="调整" /></a>
										<logic:equal name="result" property="curr_post" value="P2001">
										   &nbsp;&nbsp;<a 
										   href="#" onclick="dosubmit('<%=request.getContextPath()%>/newrisk/result/t10_checkparty_main.do?party_id=<%=party_id %>&party_class_cd=<bean:write name="result" property="party_class_cd" />')"><img src="../../images/<%=session.getAttribute("style")%>/b_pass.png" title="客户识别" /></a> 
										</logic:equal>	
									</td>
									<td>
										<a href="#" onclick="dosubmit('<%=request.getContextPath()%>/newrisk/result/t10_checkparty_main.do?resulekey=<%=resulekey%>&temptype=<bean:write name="result" property="temptype" />&party_id=<%=party_id %>&party_class_cd=<bean:write name="result" property="party_class_cd" />&organkey=<bean:write name="result" property="organkey"/>&dispatcher=_view')"><img src="../../images/<%=session.getAttribute("style")%>/b_caseinfo.png" title="详细信息" /></a> 
									</td>
									...
									<td>
								        ...
								        <td>
												...
												<bean:write name="result" property="tempcategory_disp" />
										</td>
										</logic:notEqual>
									<!--<td>
										<bean:write name="result" property="lastlevelkey" />
									</td>
									<td>
										<bean:write name="result" property="lastcheckflag" />
									</td>-->
								</tr>
							</logic:iterate>
						</logic:present>
