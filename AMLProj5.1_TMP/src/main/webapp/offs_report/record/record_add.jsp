...
function getCreate_dt_end_disp(record_type){
    ...
    if(record_type!="3"){
       ...
       if(errMsg=="" && !_compareTwoDateForString1(document.forms[0].create_dt_disp.value,document.forms[0].create_end_dt_disp.value)){
           if(record_type=="1"){
	          errMsg+= "培训时间开始时间不能大于结束时间！\n";
	       }else if(record_type=="2"){
	          errMsg+= "活动时间开始时间不能大于结束时间！\n";
	       }else if(record_type=="4"){
	          errMsg+= "审计期限开始时间不能大于结束时间！\n";
	       }else if(record_type=="5"){
	          errMsg+= "协助时间开始时间不能大于结束时间！\n";
	       }
       }
    }
   return errMsg; 
...
function getContent(record_type){
   ...
   if (getLength(document.forms[0].content.value)>1024) {
       if(record_type=="1"){
          errMsg+= "培训内容不能超过1024个字符！\n";
       }else if(record_type=="2"){
          errMsg+= "宣传内容不能超过1024个字符！\n";
       }else if(record_type=="3"){
          errMsg+= "主要内容不能超过1024个字符！\n";
       }else if(record_type=="4"){
          errMsg+= "审计项目名称及主要内容不能超过1024个字符！\n";
       }else if(record_type=="5"){
          errMsg+= "协助内容不能超过1024个字符！\n";
       }
       document.forms[0].content.focus();
   }
...
function getRemark1(record_type){
   ...
   if (getLength(document.forms[0].remark1.value)>256) {
       if(record_type=="1"){
          errMsg+= "培训对象不能超过256个字符！\n";
       }else if(record_type=="2"){
          errMsg+= "参加人数不能超过256个字符！\n";
       }else if(record_type=="3"){
          errMsg+= "制度名称不能超过256个字符！\n";
       }else if(record_type=="4"){
          errMsg+= "审计发现的主要问题不能超过256个字符！\n";
       }else if(record_type=="5"){
          errMsg+= "效果不能超过256个字符！\n";
       }
       document.forms[0].remark1.focus();
...
function getRemark2(record_type){
   ...
   if (getLength(document.forms[0].remark2.value)>256) {
       if(record_type=="1"){
          ...
          errMsg+= "发放材料份数不能超过256个字符！\n";
       }else if(record_type=="3"){
          errMsg+= "相关文号不能超过256个字符！\n";
       }else if(record_type=="5"){
          errMsg+= "协助单位(公安机关/其他机关)不能超过256个字符！\n";
       }
       document.forms[0].remark2.focus();
   }
   if(record_type=="1"){
...
function getRemark3(record_type){
   ...
   if (getLength(document.forms[0].remark3.value)>256) {
       if(record_type=="1"){
          errMsg+= "培训方式不能超过256个字符！\n";
       }else if(record_type=="2"){
          errMsg+= "宣传方式不能超过256个字符！\n";
       }else if(record_type=="4"){
          errMsg+= "问题整改情况不能超过256个字符！\n";
       }
       document.forms[0].remark3.focus();
   }
   return errMsg;  
...
function getRecord_obj(record_type){
   ...
   if (getLength(document.forms[0].record_obj.value)>256) {
       if(record_type=="3"){
          errMsg+= "制定部门不能超过256个字符！\n";
       }else if(record_type=="4"){
          errMsg+= "审计部门名称不能超过256个字符！\n";
       }else if(record_type=="5"){
          errMsg+= "被协助部门不能超过256个字符！\n";
       }
       document.forms[0].record_obj.focus();
   }
   return errMsg;  
...
<!-- conditions --> 
  <div class="conditions">
    <!-- title -->
	<div class="cond_t">
	  <span><jsp:include flush="true" page="get_record_name.jsp"></jsp:include>- 新建</span>
	  <span class="buttons">
     ...
     <jsp:include flush="true" page="record_add_index.jsp"></jsp:include>
      <tr >
        <td  align="left" class="in_button" colspan="2">
	                &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
	               &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
	               &nbsp; &nbsp;  <input type="button" value="提 交" name="Button22"  onclick="checkForm('t07_record_add_do.do?record_type_cd=<bean:write name="t07_recordActionForm" property="record_type_cd" ignore="true"/>')"/>
